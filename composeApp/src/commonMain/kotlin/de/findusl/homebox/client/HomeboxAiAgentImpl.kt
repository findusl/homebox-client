...
class HomeboxAiAgentImpl : HomeboxAiAgent {
	private val homeboxClient = generateHomeboxClient()
	override val currentLocation = MutableStateFlow<Uuid?>(null)
	override val events = MutableStateFlow<PersistentList<Event>>(persistentListOf())

	override suspend fun executeUserCommand(commandRecording: Buffer): String {
...
private fun generateHomeboxClient(): HomeboxClientImpl {
	...
	return HomeboxClientImpl(httpClient, BuildKonfig.HOMEBOX_BASE_URL, BuildKonfig.HOMEBOX_USERNAME, BuildKonfig.HOMEBOX_PASSWORD)
}
